...
using ai::openai::codex2::protocol::JsonRpcKind;
...
void ProtocolNormalizer::observeRawInbound(const nlohmann::json &message) {
  const auto method = ai::openai::codex2::protocol::jsonRpcMethod(message);
  const JsonRpcKind kind =
      ai::openai::codex2::protocol::classifyJsonRpc(message);
...
#define CODEXUI_MATCH_SERVER_REQUEST(OperationName, methodName)                \
  if (method ==                                                                \
      ai::openai::codex2::generated::server_requests::OperationName::method)   \
    return true;
  AI_OPENAI_CODEX2_SERVER_REQUESTS(CODEXUI_MATCH_SERVER_REQUEST)
#undef CODEXUI_MATCH_SERVER_REQUEST
#define CODEXUI_MATCH_SERVER_NOTIFICATION(OperationName, methodName)           \
  if (method == ai::openai::codex2::generated::server_notifications::          \
                    OperationName::method)                                     \
    return true;
  AI_OPENAI_CODEX2_SERVER_NOTIFICATIONS(CODEXUI_MATCH_SERVER_NOTIFICATION)
