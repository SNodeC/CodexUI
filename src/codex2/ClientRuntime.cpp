...
#include "codex2/Configuration.h"
#include "codex2/PresentationProtocol.h"
#include "codex2/ProtocolNormalizer.h"
...
#if defined(CODEXUI_CODEX2_FRONTEND_WEBSOCKET)
...
#if defined(CODEXUI_CODEX2_FRONTEND_TLS)
#include <net/in/stream/tls/SocketClient.h>
#include <net/in6/stream/tls/SocketClient.h>
#endif
#if defined(CODEXUI_CODEX2_FRONTEND_RFCOMM)
...
void dispatchRequest(codex2::frontend::CodexBridge &sdk,
...
int runClientRuntime(int socketPairDescriptor, Configuration &configuration,
  ...
  codex2::protocol::JsonLineFramer ipcFramer(maximumFrameBytes);
  codex2::frontend::CodexBridge sdk({});
  ...
  const auto sendToQt = [&ipcEndpoint,
    ...
    try {
      return ipcEndpoint->send(
          codex2::protocol::JsonLineFramer::encode(message, maximumFrameBytes));
    } catch (...) {
      return false;
    }
  };
  ...
  sdk.onRawJson([&normalizer](codex2::protocol::AppServerDirection direction,
...
#define CODEXUI_REGISTER_SERVER_REQUEST(OperationName, methodName)             \
  sdk.on##OperationName(                                                       \
      [&normalizer](codex2::generated::server_requests::OperationName::Params  \
                        &request) {                                            \
        normalizer.serverRequest(                                              \
            codex2::generated::server_requests::OperationName::method,         \
            request.jsonRpcId(), request.getPayload());                        \
      });
  AI_OPENAI_CODEX2_SERVER_REQUESTS(CODEXUI_REGISTER_SERVER_REQUEST)
#undef CODEXUI_REGISTER_SERVER_REQUEST

#define CODEXUI_REGISTER_SERVER_NOTIFICATION(OperationName, methodName)        \
  sdk.on##OperationName(                                                       \
      [&normalizer](                                                           \
          codex2::generated::server_notifications::OperationName::Params       \
              &notification) {                                                 \
        normalizer.serverNotification(                                         \
            codex2::generated::server_notifications::OperationName::method,    \
            notification.getPayload());                                        \
      });
  AI_OPENAI_CODEX2_SERVER_NOTIFICATIONS(CODEXUI_REGISTER_SERVER_NOTIFICATION)
#undef CODEXUI_REGISTER_SERVER_NOTIFICATION
  ...
  net::un::stream::legacy::SocketClient<StreamFactory,
      ...
      unixClient("codex-ui-codex2-unix", connection,
  ...
  net::in::stream::legacy::SocketClient<StreamFactory,
                                        client::ClientConnection &, std::size_t>
      ipv4Client("codex-ui-codex2-ipv4", connection,
                 std::size_t(maximumFrameBytes));
  ...
  net::in6::stream::legacy::SocketClient<
      ...
      ipv6Client("codex-ui-codex2-ipv6", connection,
                 std::size_t(maximumFrameBytes));
  ...
  net::in::stream::tls::SocketClient<StreamFactory, client::ClientConnection &,
                                     std::size_t>
      tlsIpv4Client("codex-ui-codex2-tls-ipv4", connection,
                    std::size_t(maximumFrameBytes));
  ...
  net::in6::stream::tls::SocketClient<StreamFactory, client::ClientConnection &,
                                      std::size_t>
      tlsIpv6Client("codex-ui-codex2-tls-ipv6", connection,
                    std::size_t(maximumFrameBytes));
  ...
  client::WebSocketHttpClient<net::in::stream::legacy::SocketClient>
      webSocketIpv4Client("codex-ui-codex2-websocket-ipv4", beginWebSocket,
  ...
  client::WebSocketHttpClient<net::in6::stream::legacy::SocketClient>
      webSocketIpv6Client("codex-ui-codex2-websocket-ipv6", beginWebSocket,
                          endWebSocket, webSocketBinding);
  ...
  client::WebSocketHttpClient<net::in::stream::tls::SocketClient> wssIpv4Client(
      "codex-ui-codex2-wss-ipv4", beginWebSocket, endWebSocket,
      webSocketBinding);
  ...
  client::WebSocketHttpClient<net::in6::stream::tls::SocketClient>
      wssIpv6Client("codex-ui-codex2-wss-ipv6", beginWebSocket, endWebSocket,
                    webSocketBinding);
  ...
  const auto dispatchCommand = [&](nlohmann::json command) {
    ...
    using namespace codex2::generated::client_requests;
