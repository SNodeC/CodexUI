...
test("C++ prompt queue isolation and callback-only acknowledgement", () => {
    ...
    const beforeDelay = projectConversation(first, prompts.submissions(first.id), 80, 1099);
    const localKey = {kind: "prompt", submissionId: firstId};
    assert.equal(findCard(beforeDelay, localKey).payload.showPendingAnimation, false);
    const afterDelay = projectConversation(first, prompts.submissions(first.id), 80, 1100);
    assert.equal(findCard(afterDelay, localKey).payload.showPendingAnimation, true);
    assert.equal(prompts.acknowledge(first.id, firstId, "turn-2"), true);
    const acknowledgedIndex = prompts.reconcile(first.id, first);
    assert.equal(prompts.submission(first.id, firstId), undefined);
    const materialized = projectConversation(acknowledgedIndex, prompts.submissions(first.id), 80, 200, first);
    assert.equal(findCard(materialized, localKey).kind, "userMessage");
    const index = indexAuthoritativeItems(first.id, first);
    prompts.reconcile(first.id, index);
...
test("C++ duplicate prompts bind in admission order and share one turn section", () => {
    ...
    prompts.beginNext(thread.id); prompts.acknowledge(thread.id, first, "turn-2");
    prompts.beginNext(thread.id, "turn-2"); prompts.acknowledge(thread.id, second, "turn-2");
    addTurn(thread, "turn-2");
    ...
    const promoted = prompts.reconcile(thread.id, thread);
    assert.equal(prompts.submission(thread.id, first), undefined);
    assert.equal(prompts.submission(thread.id, second), undefined);
    const snapshot = projectConversation(promoted, prompts.submissions(thread.id), 80, 1021, thread);
